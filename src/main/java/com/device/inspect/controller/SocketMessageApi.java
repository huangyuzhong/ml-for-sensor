                    ...
                    for (DealRecord dealRecord : dealRecords) {
                        ...
                        if(deviceRunningStatusHistories != null && deviceRunningStatusHistories.size() > 0){
                            for(List<Object> deviceRunningStatusHistory : deviceRunningStatusHistories){
                                LOGGER.info(String.format("device %d change status to %d at %s", device.getId(), (Integer)deviceRunningStatusHistory.get(1), new Date((long)deviceRunningStatusHistory.get(0)));
                                if((Integer)deviceRunningStatusHistory.get(1) == 20){
                                // device has run
